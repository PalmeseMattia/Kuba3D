...
void sprites_draw(t_cube *cube)
{
	...
	// Render the exit
	if (entities->exit) {
		t_exit *exit = entities->exit;

		// Translate exit position relative to camera
		double exit_x = exit->x - player->x;
		double exit_y = exit->y - player->y;

		// Transform exit with the inverse camera matrix
		double inv_det = 1.0 / (player->camera.dir_x * player->dir.dir_y - 
							   player->dir.dir_x * player->camera.dir_y);

		double transform_x = inv_det * (player->dir.dir_y * exit_x - 
									  player->dir.dir_x * exit_y);
		double transform_y = inv_det * (-player->camera.dir_y * exit_x + 
									  player->camera.dir_x * exit_y);

		// Calculate exit's screen position
		int exit_screen_x = (int)((WINDOW_WIDTH / 2) * (1 + transform_x / transform_y));

		// Calculate exit dimensions on screen
		int exit_height = abs((int)(WINDOW_HEIGHT / transform_y));
		int exit_width = exit_height; // Assuming square exit texture

		// Calculate exit drawing boundaries
		int draw_start_y = -exit_height / 2 + WINDOW_HEIGHT / 2;
		if (draw_start_y < 0) draw_start_y = 0;

		int draw_end_y = exit_height / 2 + WINDOW_HEIGHT / 2;
		if (draw_end_y >= WINDOW_HEIGHT) draw_end_y = WINDOW_HEIGHT - 1;

		int draw_start_x = -exit_width / 2 + exit_screen_x;
		if (draw_start_x < 0) draw_start_x = 0;

		int draw_end_x = exit_width / 2 + exit_screen_x;
		if (draw_end_x >= WINDOW_WIDTH) draw_end_x = WINDOW_WIDTH - 1;

		// Draw the exit
		for (int stripe = draw_start_x; stripe < draw_end_x; stripe++) {
			// Skip if exit is behind a wall
			if (transform_y <= 0 || stripe < 0 || stripe >= WINDOW_WIDTH || 
				transform_y >= entities->buffer_z[stripe])
				continue;

			// Calculate texture x-coordinate
			int tex_x = (int)(256 * (stripe - (-exit_width / 2 + exit_screen_x)) * 
							TEXTURE_SIZE / exit_width) / 256;

			// Draw vertical stripe of the exit
			for (int y = draw_start_y; y < draw_end_y; y++) {
				// Calculate texture y-coordinate
				int d = (y * 256) - (WINDOW_HEIGHT * 128) + (exit_height * 128);
				int tex_y = ((d * TEXTURE_SIZE) / exit_height) / 256;

				// Get color from exit texture
				unsigned int color = 0;
				int frame = exit->animation_controller->idle->frame;
				tex = exit->animation_controller->idle->frames_ptr->frames[frame];
				if (tex)
					color = tex[TEXTURE_SIZE * tex_y + tex_x];
				// Draw pixel if it's not transparent (assuming 0 is transparent)
				if ((color & 0x00FFFFFF) != 0)
